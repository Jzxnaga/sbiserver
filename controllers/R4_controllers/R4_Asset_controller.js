...
const { R4_Asset , R4_PU77 , R4_TT38 } = require ("../../models");
...
class R4_Asset_controller{
  ...
  static async findAllType(req,res,next){
    R4_Asset.findAll({where:{merk:req.body.merk}})
    .then(data=>{
      res.status(200).json(data)
    })
    .catch(err=>{
  ...
  static async findAllModel(req,res,next){
    R4_Asset.findAll({where:{merk:req.body.merk , tipe:req.body.tipe}})
    .then(data=>{
      res.status(200).json(data)
    })
    .catch(err=>{
  ...
  static async findAssetLessorAsId(req,res,next){
    ...
    if (req.body.lessor == "R4_PU77"){
      using_db = R4_PU77
    }else if (req.body.lessor == "R4_TT38"){
      using_db = R4_TT38
    }

    using_db.findOne({include: [{ model: R4_Asset , 
  ...
  static async findAssetLessorAsIdManyDb(req,res,next){
    ...
    for (var i = 0 ; i<arrayOfLessors.length ; i++){
      ...
      if (arrayOfLessors[i] == "R4_PU77"){
        using_db = R4_PU77
      }else if (arrayOfLessors[i] == "R4_TT38"){
        using_db = R4_TT38
      }else if (arrayOfLessors[i] == "R4_PT74"){
        using_db = R4_PT74
      }else if (arrayOfLessors[i] == "R4_PB63"){
        using_db = R4_PB63
      }else if (arrayOfLessors[i] == "R4_BU42"){
        using_db = R4_BU42
      }else if (arrayOfLessors[i] == "R4_AL39"){
        using_db = R4_AL39
      }
      
      await using_db.findOne({include: [{ model: R4_Asset , 
      where :{id : req.body.idSelected}}]})
      .then(data=>{
...
module.exports = R4_Asset_controller;